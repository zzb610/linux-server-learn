...
http_conn::HTTP_CODE http_conn::parse_headers(char *text)
{
    ...
    else if (strncasecmp(text, "Connection:", 1) == 0)
    {
        text += 1;
        text += strspn(text, " \t");
...
bool http_conn::write()
{
    ...
    if (bytes_to_send == 0)
    {
        mod_fd(m_epoll_fd, m_sockfd, EPOLLIN);
        init();
        return true;
    }
    while (true)
    {
        ...
        if (temp <= -1)
        {
            if (errno == EAGAIN)
            {
                ...
                mod_fd(m_epoll_fd, m_sockfd, EPOLLOUT);
                return true;
            }
            unmap();
            return false;
...
// called by thread in thread pool
